site_data <- function (data, category, portfolio) 
{
  ...
  if (data == "COE") {
    ...
    data_set$year <- NULL
    data_set$day <- NULL
    colnames(data_set) = gsub("yield_week_pc", "dividend_yield", colnames(data_set))
  }
    ...
    data_set$month <- NULL
    data_set$day <- NULL
    colnames(data_set) = gsub("week_loan_fee", "loan_fee", colnames(data_set))
  }
    ...
    data_set$month <- NULL
    data_set$day <- NULL
    colnames(data_set) = gsub("week_si", "short_interest", colnames(data_set))
  }
  else {
    if (category == "IVolBR") {
      date <- as.Date(paste(data_set$year, data_set$month, 
